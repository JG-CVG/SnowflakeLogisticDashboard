...
## CAR2 — CarAudit Closed reason breakdown (weekly, Phase 2)  (canvas `car2`, `const CAR2`)  ✅ IMPLEMENTOVÁNO 26.06.2026
**Zdroj:** ☁ Salesforce. Skill: `ca-closed-top-reasons`. `const CAR2=[wk, car_na, car_cond, seller, auto, client, carvago, other]` (ISO týden, oldest first; chart reverse). Od ISO týdne 47/25.

### Definice (ověřeno 1:1 proti const — settled týdny 47/25–2/26 přesně)
- Populace = Phase 2 closed: `CarAudit_Status__c LIKE '%REJECT%' AND NOT IN` (4 Phase 1 rejecty), RecordType CarAudit. Týden = ISO týden `CA_New_CarAudit_Date__c`.
- Kategorie = `map_reason_ca(CA Reason Code)` (pořadí check DŮLEŽITÉ): car_na → client → car_cond → seller → auto ('automatically reject'/'closed order') → carvago → other. ⚠️ BEZ Sales-LOST mappingu (na rozdíl od CATOP) — kategorizuje se raw reason.
- Reason zdroj = picklist `CarAudit__r.Reason_Code__c` (≡ CA Reason Code).

### Implementace
`build_car2(records)` v `scripts/build_sf_views.py` (gated na sf_car2.json; guard total<200). Vstup = denní agregace, ISO týden + kategorie v Pythonu.

### SOQL (sf_car2.json)
```
SELECT DAY_ONLY(CA_New_CarAudit_Date__c) d, CarAudit__r.Reason_Code__c rc, COUNT(Id) c
FROM Case
WHERE RecordType.Name IN ('CarAudit','Carvago CarAudit')
  AND CarAudit_Status__c LIKE '%REJECT%'
  AND CarAudit_Status__c NOT IN ('REJECT New CA','REJECT Data Validation','REJECT Car Check','REJECT VIN Check')
  AND CA_New_CarAudit_Date__c >= 2025-11-17T00:00:00Z
GROUP BY DAY_ONLY(CA_New_CarAudit_Date__c), CarAudit__r.Reason_Code__c
```
(2025-11-17 = pondělí ISO týdne 47/25 → graf začíná 47/25.) Pozn.: FCR (Phase 1 weekly breakdown) zůstává ❄ Snowflake — není součástí tohoto SF kroku.
